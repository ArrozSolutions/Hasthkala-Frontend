            ...
            <div className='flex flex-col relative z-900 min-h-full'>
                ...
                <div className={`${showFilter ? 'left-0' : 'left-[-380px]'} transition-all duration-200 block sm:hidden fixed top-[-25px] left-0 z-[1000] pt-4 pl-4 pr-4 bg-white w-[320px] shadow-2xl min-h-full mt-6  rounded`}>
                    <div className='h-10 flex' onClick={toggleShowFilter}>
                        <p className='flex text-darkred items-center pb-5'><BiX size={18} />close</p>
                    </div>
                            ...
                            <div key={key} className='mb-2 flex items-center justify-between'>
                                <div className='flex flex-col w-full'>
                                    <div className='flex w-full justify-between mb-1'>
                                        ...
                                        <div className='flex'>
                                            <input className='h-[15px] w-[15px]' onChange={(e) => { handleFilter(e.target.checked, category._id, 'parent') }} type="checkbox" />
                                    ...
                                    {(category?._id == current) && (category?.children?.length > 0) && <div className=' min-h-20 w-full'>
                                        {
                                            category?.children?.map((child, key) => (
                                                <div key={key} className='flex'>
                                                    <input className='h-[13px] w-[13px] ml-4' onChange={(e) => { handleFilter(e.target.checked, child._id, 'child') }} type="checkbox" />
                    ...
                    <div className='flex flex-col sm:min-h-full w-full'>
                        <div className='mt-10 h-5 w-full flex justify-between sm:justify-end text-sm text-gray font-dmsans sm:pr-10 items-center pl-5 sm:pl-0'>
                            <div onClick={toggleShowFilter} className='block sm:hidden'>
                                <p className='flex items-center hover:text-darkred'>Filters<BiChevronDown /></p>
                            </div>
                        ...
                        <div className='flex flex-col justify-between'>
                            <div className='sm:min-h-full pt-8 pl-0 pr-0 sm:pl-14 max-w-full grid grid-cols-2 sm:grid-cols-3 sm:gap-y-8 mb-10'>
                                ...
                                remainingProducts > 0 &&
                                <div className='scale-95 sm:scale-100 w-full h-20 flex justify-end items-start rounded-lg shadow' onClick={handlePage}><button className='bg-darkred text-white sm:w-[120px] px-2 mr-2 sm:mr-0 h-[32px] font-dmsans mb-20 flex justify-center items-center'>Load More <BiLoader className='ml-1' size={18} /></button></div>
                            }
                        </div>
                    </div>
                </div>
