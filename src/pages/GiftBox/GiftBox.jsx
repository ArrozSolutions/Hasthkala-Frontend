...
const GiftBox = () => {
  ...
  const handleCurrentPage = () => {
    if (currentPage == null) {
      ...
      selectedProduct?setCurrentPage('card'):errorToast("Select A Product!");
    }
            ...
            <div className='w-full grid grid-cols-2 pl-2 pr-2 xl:grid-cols-3 gap-x-2 sm:gap-x-0'>
              {
                giftCards?.map((card, key) => (
                  <div key={key} className='h-[200px] sm:h-[310px] cursor-pointer shadow-xl sm:w-[248px] border rounded-lg flex flex-col justify-start  pb-3 relative gap-y-5 pl-2' onClick={() => { selectCard(card) }}>
                    <div className='absolute right-3 top-3'>
                      {
                    ...
                    <div className='h-[130px] sm:h-[250px] w-full overflow-hidden'><img className='h-full w-full' src={card?.images[0]?.img} alt="" /></div>
                    <div>
                      <p className='pl-2 font-dmsans pt-2 text-xs sm:text-sm'>{card?.name}</p>
