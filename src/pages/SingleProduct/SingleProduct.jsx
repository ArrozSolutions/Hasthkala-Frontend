...
const SingleProduct = () => {
    ...
    const similarproducts = useSelector(state => state.products.relatedproducts);
        ...
        <>
            ...
            <div className='flex flex-col relative'>
                ...
                {product ? <div className='flex flex-col sm:flex-row min-h-[800px]'>
                    {popup && <div className='w-full flex justify-center items-center h-screen bg-[#8383837d] shadow-2xl fixed top-0 z-[1000]'>
                        <div className='w-[500px] h-[330px] sm:scale-100 scale-90 cursor-pointer bg-white shadow-2xl rounded-xl'>
                            {product?.personalizationType == "engravement" &&
                                <div className='flex w-full h-full flex-col relative'>
                                    <div onClick={handlePopup} className='absolute right-4 top-4'><BiXCircle size={24} color='white' /></div>
                                    <p className='font-dmsans pl-4 mb-8 h-14 flex items-center bg-darkred rounded-t-xl text-white'>Personalization</p>
                                    <div className='flex items-center mb-2'>
                                        <p className='font-semibold pl-4 pr-5'>Image</p><input type="file" onChange={handleImagesSelect} className='rounded w-24' />
                                        {blobImg && <div className='h-[120px] w-[120px] ml-5'>
                                            <img className='h-full w-full' src={blobImg} alt="" />
                                        </div>}
                                    </div>
                                    <div className='flex items-center'>
                                        <p className='pl-4 pr-2 sm:pl-4 sm:pr-5 font-dmsans'>Text</p><input onChange={(e) => { setCustomText(e.target.value) }} type="text" className='w-[300px] rounded h-9 text-sm' placeholder='Enter Custom Text' />
                                    </div>
                                    {
...

                        {
                            ...
                            <div className='flex flex-col mt-5'>
                                <div className='flex flex-col'>
                                    ...
                                    <div className='flex'>
                                        {product?.variants?.map((v, key) => (
                                            v?.attribute == "color" && <div onClick={() => { setPrice(v?.price) }} className='h-9 w-20 border rounded border-[#bebebe] font-dmsans flex justify-center items-center text-lg mr-2'>{v?.name}</div>
                                        ))}
                                    </div>
                                </div>
                            </div>
                ...
                {relatedproduct && <div><SimilarProducts products={relatedproduct} /></div>}
