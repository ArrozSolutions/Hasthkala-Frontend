        ...
        <>
            {(cartdata.length >= 1) ?
                (cartdata?.map((c, key) => (
                    ...
                    }} key={key} className='flex bg-white w-full cursor-pointer h-[100px]  sm:h-[150px] items-center pl-2 sm:pl-5 justify-between sm:pr-10 pr-4 shadow-lg'>
                        <div className='flex items-center'>
                            <div className='sm:w-24 sm:h-24 w-16 h-16 mr-5'>
                                <img className='rounded h-full w-full mr-3' src={c?.productid?.images[0]?.img || c[0]?.product?.images[0]?.img} alt="" />
                            </div>
                    ...
                    <div className='flex items-center'>
                        <div className='sm:w-24 sm:h-24 w-16 h-16 mr-5'>
                            <img className='rounded h-full w-full mr-3' src={cartdata?.images[0]?.img || cartdata?.productid?.images[0]?.img} alt="" />
                        </div>
                        <div className='flex flex-col'>
                            <h1 className='font-dmsans block sm:hidden text-[13px] mb-[5px] sm:mb-0 sm:text-[16px]'>Deliverd on Sep 15</h1>
                            <h1 className='text-[#333337]  text-[14px] sm:text-[16px]'>{cartdata?.name || cartdata?.productid?.name}</h1>
                            <p className='text-[#787878] text-[11px] sm:text-xs'>Color: Cream Size: 22x24cm</p>
                            {customimg && <p className='text-[10px] sm:text-xs sm:mt-2 flex'><BiCheckCircle color='green' className='mr-1' size={18} />Custom Img</p>}
                            {customtext != undefined && <p className='text-[10px] sm:text-xs sm:mt-2 flex'><BiCheckCircle color='green' className='mr-1' size={18} />Custom Text:&nbsp;{customtext}</p>}
                            {(customlink != undefined) && (customlink != 'null') && <p className='text-[10px] sm:text-xs sm:mt-2 flex'><BiCheckCircle size={18} className='mr-1' color='green' />Custom Link:&nbsp;{customlink}</p>}
                        </div>
                    </div>
                    <div className='h-full pt-10'>
                        <p className='font-dmsans '>Mode:&nbsp;<span className='text-green-500 font-dmsans'>{paymentmode}</span></p>
